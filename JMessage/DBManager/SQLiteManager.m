...
- (NSError *)doQuery:(NSString *)sql {
	...
	if (openError == nil) {
		...
		int nError = sqlite3_prepare_v2(db, query, -1, &statement, NULL);
		if (nError == SQLITE_ERROR || statement == nil) {
			const char *errorMsg = sqlite3_errmsg(db);
            ...
            NSLog(@"sqlite3_prepare_v2 error: %@", errorQuery);
		} else {
            if (sqlite3_step(statement) == SQLITE_ERROR) {
                const char *errorMsg = sqlite3_errmsg(db);
                errorQuery = [self createDBErrorWithDescription:[NSString stringWithCString:errorMsg encoding:NSUTF8StringEncoding]
                                                        andCode:kDBErrorQuery];
                NSLog(@"sqlite3_step error: %@", errorQuery);
            }
            sqlite3_finalize(statement);
        }

        closeQuery = [self closeDatabase];
        if (errorQuery == nil)
            errorQuery = closeQuery;
	}
	else {
...
-(void) getColumeInfo:(NSString *)sql {
    if (self.columnArray == nil)
        self.columnArray = [NSMutableArray new];
    ...
    if ( statement )
    {
        ...
        for (int i = 0; i < columnCount; i++)
        {
            ...
            [self.columnArray addObject:fieldName];
        }
    }
...
{
    NSUInteger nIndex = [self.columnArray indexOfObject:fieldName];
